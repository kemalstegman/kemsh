...
pub trait LexableParsableCharBuffer {
    ...
    /// this will affect subsequent `.chars()` calls
    /// and should be offset by the consumed amount
    fn lex_consume(&mut self, chars: usize);
    /// the lex consumption needs to be reversible by the
    /// parser. the parser will either assert consumption
    /// or reset the consumption.
    fn parse_consume(&mut self);
    /// the lex consumption needs to be reversible by the
    /// parser. the parser will either assert consumption
    /// or reset the consumption.
    fn parse_regurgitate(&mut self);
}
